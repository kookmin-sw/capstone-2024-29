...
def train(epochs, dataloader, model, criterion_L1, criterion_ssim, optimizer, opt):
    ...
    for epoch in epochs:
        total_loss = 0.0
        for images, masks in enumerate(dataloader):
            ...
            RECON_loss += 1*criterion_L1(frame_o1, frame_i[0]) - criterion_ssim(frame_o1, frame_i[0])
            HOLE_loss += 5*criterion_L1(
                frame_o1*frame_m[0][:,:,4,:,:].expand_as(frame_o1), 
                frame_i[0]*frame_m[0][:,:,4,:,:].expand_as(frame_o1)
                )

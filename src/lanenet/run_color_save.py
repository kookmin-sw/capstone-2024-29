...
def main():
    ...
    # 비디오 저장 설정
    output_path = './output.mp4'  # 출력 비디오 경로
    fourcc = cv2.VideoWriter_fourcc(*'mp4v')  # 비디오 코덱 설정
    fps = video_capture.get(cv2.CAP_PROP_FPS)  # 원본 비디오의 FPS 가져오기
    width = int(video_capture.get(cv2.CAP_PROP_FRAME_WIDTH))  # 원본 비디오의 너비 가져오기
    height = int(video_capture.get(cv2.CAP_PROP_FRAME_HEIGHT))  # 원본 비디오의 높이 가져오기
    video_writer = cv2.VideoWriter(output_path, fourcc, fps, (width, height))  # 비디오 저장 객체 생성
    ...
    while video_capture.isOpened():  # 비디오 프레임 반복
        ...
        if ret:
            ...
            video_writer.write(processed_frame)  # 처리된 프레임을 비디오 파일에 저장
    ...
    video_writer.release()  # 비디오 저장 객체 해제
